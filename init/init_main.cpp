...
int main(int argc, char** __attribute__((unused)) argv)
{
    ...
    if (!recoveryBoot)
    {
        ...
        keycheckStatus = system_exec_kill(keycheck_pid);
    ...
    if (recoveryBoot)
    {
        ...
        if (DEV_FOTA_MAJOR != -1 && DEV_FOTA_MINOR != -1 &&
                keycheckStatus != KEYCHECK_RECOVERY_BOOT)
        {
            mknod(DEV_FOTA_PATH, S_IFBLK | 0600,
                    makedev(DEV_FOTA_MAJOR, DEV_FOTA_MINOR));
            mount("/", "/", NULL, MS_MGC_VAL | MS_REMOUNT, "");
            const char* argv_extract_elf[] = { "", "-i", DEV_FOTA_PATH, "-o",
                    SBIN_CPIO_RECOVERY, "-t", "/", "-c" };
